...
static void drawStuff()
{
    ...
    const RigTForm eyeRbt = *g_frames[g_frame_index];
...
static void motion(GLFWwindow *window, double x, double y)
{
    ...
    // Cvec3 eyecoord =  (g_arcballRbt).getTranslation() - g_frames[g_frame_index]->getTranslation();
    Cvec3 eyecoord = (inv(*g_frames[g_frame_index]) * g_arcballRbt).getTranslation();
    ...
    { // left button down?
        ...
        else
        {
            ...
            Quat rot = Quat(0, v_2) * Quat(0, v_1 * -1);
            m = rbtMtoOwrtA(RigTForm(rot), OT * ER);
            // m = RigTForm(rot);
        }
    }
