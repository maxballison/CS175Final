...
inline Matrix4 transFact(const Matrix4 &m) {
    //return identity matrix with translation factors of m
    Matrix4 l = Matrix4();
    for (int r = 0; r < 3; r++) {
        l(r,3) = m(r, 3);
    }
...
inline Matrix4 linFact(const Matrix4 &m) {
    // return m for the top left 3 x 3 square and 1 in bottom right. 
    Matrix4 l = Matrix4();
    for (int r = 0; r < 3; ++r) {
        for(int c = 0; c < 3; ++c) {
            l(r, c) = m(r, c);
...
inline Matrix4 doMtoOwrtA(const Matrix4 &m, const Matrix4 &a) {
    Matrix4 a_inv = inv(a);
    return a * m * a_inv;
}
